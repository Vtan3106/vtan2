...
try {
    ...
    logger.loader("Tiến hành tải tệp config: config.json");
}
catch {
    ...
    else return logger.loader("Lỗi file config.json, hãy kiểm tra lại", "error");
}
...
catch { return logger.loader("Không thể tải tệp confing.json", "error") }
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
      ...
      //  logger.loader('Ping Load Source Code: ' + (Date.now() - global.client.timeStart) + 'ms')
        ...
        console.log(chalk.bold.hex("#008080").bold(``));
...
var myVar = setInterval(rest,  1000 * 420 * 60);
