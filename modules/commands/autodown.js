...
async function noprefix(arg) {
    ...
    for (const el of arr) {
        ...
        if (regEx_youtube.test(el)) {
            const data = (await axios.get(`https://nguyenmanh.name.vn/api/autolink?url=${el}&apikey=sKJE8Sg6`)).data.data,
        ...
        if (regEx_instagram.test(el))out({
            body: '=== [ 𝐈𝐍𝐒𝐓𝐀𝐆𝐑𝐀𝐌 ] ===\n━━━━━━━━━━━━\n\n🌸 𝗖𝗵𝘂̛́𝗰 𝗻𝗮̆𝗻𝗴 𝗮𝘂𝘁𝗼𝗱𝗼𝘄𝗻 𝗸𝗵𝗶 𝗻𝗵𝗮̣̂𝗻 𝗹𝗶𝗻𝗸 𝘁𝗿𝗼𝗻𝗴 𝗻𝗵𝗼́𝗺\n👉 𝗧𝗵𝗮̉ 𝗰𝗮̉𝗺 𝘅𝘂́𝗰 "👌" 𝗻𝗲̂́𝘂 𝗺𝘂𝗼̂́𝗻 𝘁𝗮̉𝗶 𝗻𝗵𝗮̣𝗰', attachment: await streamURL((idl = (await axios.get(`https://nguyenmanh.name.vn/api/autolink?url=${el}&apikey=sKJE8Sg6`)).data, idl[((irx = /\/p\//.test(el))?'display': 'video')+'_url']), irx?'jpg': 'mp4')});
    };
};
