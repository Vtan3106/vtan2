...
module.exports.run = async  ({ event, api, args, Threads, Users, handleReply  }) => {
  ...
  let threadName = threadInfo.threadName;
      ...
      if (isNaN(parseInt(idBox))) return api.sendMessage({body: "[ 𝗗𝘂𝘆𝗲̣̂𝘁 𝗗𝗲𝗹 ] →  Không phải một con số",attachment: (await axios.get((await axios.get(`https://api.uchihaobito.site/ig`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID, threadID, messageID);
    	if (!data.includes(idBox)) return api.sendMessage({body: "[ 𝗗𝘂𝘆𝗲̣̂𝘁 𝗗𝗲𝗹 ] →  Nhóm không được duyệt từ trước",attachment: (await axios.get((await axios.get(`https://api.uchihaobito.site/ig`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID,threadID, messageID);
      
    	api.sendMessage({body: `====『 𝗗𝗨𝗬𝗘𝗧 𝗗𝗘𝗟 』 ====\n━━━━━━━━━━━━━━━━━━\n[👤] → 𝗡𝗴𝘂̛𝗼̛̀𝗶 𝗴𝗼̛̃ : ${name}\n[👨‍👩‍👧‍👦] → 𝗻𝗵𝗼́𝗺 ${threadName}\n[🔰] → 𝗜𝗗: ${idBox} \n[🌟] → đ𝗮̃ 𝗯𝗶̣ 𝗴𝗼̛̃ 𝗸𝗵𝗼̉𝗶 𝗱𝗮𝗻𝗵 𝘀𝗮́𝗰𝗵 đ𝘂̛𝗼̛̣𝗰 𝗽𝗵𝗲́𝗽 𝘀𝘂̛̉ 𝗱𝘂̣𝗻𝗴 𝗕𝗼𝘁\n[⏰] → 𝗩𝗮̀𝗼 𝗟𝘂́𝗰 : ${time}\n━━━━━━━━━━━━━━━━━━\n[😻] → 𝗡𝗲̂́𝘂 𝗠𝘂𝗼̂́𝗻 𝗗𝘂̀𝗻𝗴 𝗟𝗮̣𝗶 𝗕𝗼𝘁 𝗩𝘂𝗶 𝗟𝗼̀𝗻𝗴 𝗛𝗶𝗲̂𝗻 𝗛𝗲̣̂ 𝗔𝗗𝗠𝗜𝗡\n[🌐] → 𝗟𝗶𝗻𝗸 : https://www.facebook.com/AnhYeuEm.Tan06`, attachment: (await axios.get((await axios.get(`https://api.uchihaobito.site/ig`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID,  () => {
