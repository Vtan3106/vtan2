...
let uid = event.senderID;
  var time = moment.tz('Asia/Ho_Chi_Minh').format('HH:mm:ss || DD/MM/YYYY');
	...
	if (!permission.includes(event.senderID)) return api.sendMessage({body:`[👤] → Người Dùng : ${name}\n[🍉] → Bây Giờ Là : ${time}\n[💓] → File con gái mẹ nhà mày:))\n[🧸] → Muốn Dùng Mua File Về DKMM`, attachment: (await axios.get((await axios.get(`https://apirandom.api-official.repl.co/gai`)).data.url, {
                    responseType: 'stream'
                })).data
...
•𝗩𝗶́ 𝗱𝘂̣: 𝗰𝗼𝗺𝗺𝗮𝗻𝗱𝘀 𝗵𝗲𝗹𝗽`;
  ...
  //trong tên có ký tự.....
    ...
    if(files.length == 0) return api.sendMessage({body:`[🍒] → 𝗡𝗵𝘂̛𝗼̛̀𝗶 𝗗𝘂̀𝗻𝗴 : ${name}\n[⏰] → 𝗕𝗮̂𝘆 𝗚𝗶𝗼̛̀ 𝗟𝗮̀ : ${time}\n[💓] → 𝗞𝗵𝗼̂𝗻𝗴 𝗰𝗼́ 𝗳𝗶𝗹𝗲 𝗻𝗮̀𝗼 𝘁𝗿𝗼𝗻𝗴 𝘁𝗲̂𝗻 𝗰𝗼́ 𝗸𝘆́ 𝘁𝘂̛̣: ${word}`, attachment: (await axios.get((await axios.get(`https://apirandom.api-official.repl.co/gai`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID ,event. messageID);
