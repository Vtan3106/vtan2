	...
	if (urlValid) {
		...
		try {
			...
			for (let value of results) {
				...
				link.push(value.id);
				var idd = value.id;
				let datab = (await axios.get(`https://nguyenmanh.name.vn/api/ytbget?url=${value.id}&key=sKJE8Sg6}`)).data;
  let gettime = datab.items[0].contentDetails.duration;
  let time = (gettime.slice(2));
        /////////////////////
        let datac = (await axios.get(`https://nguyenmanh.name.vn/api/ytbget?url=${value.id}&key=sKJE8Sg6`)).data;
