...
module.exports.handleReply = async ({ handleReply, event, api }) => {
  ...
  api.sendMessage(`» Đã ${args[0] == '𝐱𝐨́𝐚' ? '𝐜𝐡𝐚̂́𝐩 𝐧𝐡𝐚̣̂𝐧' : '𝐚𝐝𝐝'} 𝐥𝐨̛̀𝐢 𝐦𝐨̛̀𝐢 𝐤𝐞̂́𝐭 𝐛𝐚̣𝐧 𝐭𝐡𝐚̀𝐧𝐡 𝐜𝐨̂𝐧𝐠 𝐜𝐮̉𝐚 ${success.length} 𝐧𝐠𝐮̛𝐨̛̀𝐢:\n${success.join("\n")}${failed.length > 0 ? `\n» 𝐓𝐡𝐚̂́𝐭 𝐛𝐚̣𝐢 𝐯𝐨̛́𝐢 ${failed.length} 𝐧𝐠𝐮̛𝐨̛̀𝐢: ${failed.join("\n")}` : ""}`, event.threadID, event.messageID);
};
  ...
  for (const user of listRequest) {
	...
	for (ID of listRequest) {
		var
			...
			url = (await axios.get(`https://graph.facebook.com/${ID.node.id}/picture?width=720&height=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`, {responseType: "arraybuffer"})).data
		fs.writeFileSync(path, Buffer.from(url, "utf-8"))
		attachment.push(fs.createReadStream(path))
		files.push(path)
	}
  api.sendMessage({body:`${msg}\n[👉] → 𝐑𝐞𝐩𝐥𝐲 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐧𝐚̀𝐲 𝐯𝐨̛́𝐢 𝐧𝐨̣̂𝐢 𝐝𝐮𝐧𝐠: <𝐚𝐝𝐝 | 𝐝𝐞𝐥> <𝐬𝐨̂́ 𝐭𝐡𝐮̛́ 𝐭𝐮̛̣ | 𝐡𝐨𝐚̣̆𝐜 \"𝐚𝐥𝐥\"> đ𝐞̂̉ 𝐭𝐡𝐮̛̣𝐜 𝐡𝐢𝐞̣̂𝐧 𝐡𝐚̀𝐧𝐡 đ𝐨̣̂𝐧𝐠`, attachment },event.threadID, (e, info) => {
